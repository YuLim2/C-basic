...
class SalesWorker: public FullTimeWorker
{
    private:
        ...
        float bonusRatio;
    public:
        SalesWorker(char* name, int salary, float bonusRatio)
            : FullTimeWorker(name, salary), bonusRatio(bonusRatio) {}
        void AddSalesResult(int money)
        {
            performance+=money;
        }
        virtual int GetPay() const
	    {
		    return FullTimeWorker::GetPay() + (performance*bonusRatio);
	    }
	    virtual void ShowSalaryInfo() const
	    {
		    ShowYourName();
